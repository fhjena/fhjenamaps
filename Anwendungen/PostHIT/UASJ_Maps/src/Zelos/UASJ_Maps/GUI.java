...
public class GUI extends Activity {
    ...
    private short activityState = 0; // Nummer des momentan/zu letzt aktiven Status; siehe State-übersicht
    ...
    private OnTouchListener touch_single_multi = new OnTouchListener() {
        public boolean onTouch(View v, MotionEvent event) {
            if (output.isStateCampus()) { // Wird momentan der Campus dargestellt?
                ...
                } else if (event.getAction() == MotionEvent.ACTION_UP && Math.abs(event.getX()-mPosX)<10 && Math.abs(event.getY()-mPosY)<10) // Finger verlässt Touchscreen && weder in x noch y Richtung mehr als 10 Pixel bewegt
                ...
                case MotionEvent.ACTION_MOVE: {
                    ...
                    if (mode == DRAG) {                //Modus "Verschieben" aktiv
                       ...
                       if (cl.getEnabledByOptions()) {
	                       ...
	                       if((mPosX+dx)>-400*Math.sqrt(mScaleFactor) && (mPosX+dx)<550*Math.sqrt(mScaleFactor)) // TODO 550 etwa Displayhöhe/1,5
    ...
    public void onCreate(Bundle savedInstanceState) { // App wird gestartet
        ...
        new AsyncTask<Void, Void, Void>() { // neuen Task erstellen zum Anlegen der DB
            ...
            protected Void doInBackground(Void... params) { // Aufgabe des AsyncTask
                ...
                if ((folder.exists() && !file.exists()) || folder.mkdirs()) { // (wenn Ordner vorhanden und Datei nicht vorhanden) oder Ordner Anlegen erfolgreich
	                ...
	                finally { // wird nach try ausgeführt
	                    if (is != null)
	                        try {
	                            is.close(); // Inputstrem schließen
	                        } catch (IOException e) {}
	                    if (fos != null)
	                        try {
	                            fos.close(); // Outputstrem schließen
	                        } catch (IOException e) {}
	                }
                }
                return null;
        ...
        new AsyncTask<Void, Void, Void>() { // neuen Task erstellen für Anzeige Splashscreen
            ...
            protected Void doInBackground(Void... params) { // Aufgabe des AsyncTask
				...
				runOnUiThread(new Runnable() { // Ausführung an UI Thread weitergeben
					// Views können nur vom UI Thread verändert werden, ansonsten wird eine Exception ausgelöst
					public void run() {
		                ((TextView) findViewById(R.id.textView0)).setText("Starting..."); // Text auf Splashscreen ändern
					}
				});
            ...
            protected void onPostExecute(Void params) { // wird ausgeführt nachdem doInBackground fertig ist
                if (0 == activityState) // Wird noch immer der Splashscreen angezeigt?
                    launch_state_1(); // Hauptmenü anzeigen
            }
        }
        .execute(null,null,null); // Task starten; keine Übergabeparameter
    }
    ...
    /**enabled die zum activityState gehörenden F-, F+ und Campus Button
     * aktualisiert zudem die Anzeige von Haus und Etage oben links
     * @param HouseID für set_floor von GraphicalOutput
     * @return true, wenn Haus vefügbar ist; ansonsten false
     */
    ...
    private void launch_state_2() { // zeigt Look up Location (B3) an
        ...
        go.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (pf.compute_route(roomInput,roomInput)) // "Route berechnen"; wobei hier nur der Knoten für den Raum roomInput ermittelt werden soll; Location gefunden?
    ...
    private void launch_state_3() { // zeigt Look up Location Output (B4) an
        ...
        showlocation.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                short merk = output.set_floor(3); // Anzeige auf aktuelle Position; Rückgabewert merken
                if (1 == merk) // wenn 1 returned wird, oberstes Stockwerk erreicht
                    fplus.setEnabled(false); // F+ disabeln
                else if (-1 == merk) // wenn -1 returned wird, unterstes Stockwerk erreicht
                    fminus.setEnabled(false); // F- disabeln
        ...
        cl.onResume(); // Lagesensor wieder ausführen
        cl.setEnabled(true); // Compass aktivieren
    }
    ...
    private void launch_state_6() { // zeigt Options (B2) an
        ...
        ArrayAdapter<CharSequence> AA = ArrayAdapter.createFromResource(getApplicationContext(), R.array.Degree, android.R.layout.simple_spinner_item); // String Array auswählen
        ...
        findViewById(R.id.but_refreshDB6).setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) { // alte DB löschen; neue DB anlegen; SplashScreen anzeigen
                ...
                new AsyncTask<Void, Void, Void>() { // neuen Task erstellen für Anzeige Splashscreen
                    ...
                    @Override
                    protected void onPostExecute(Void params) { // wird ausgeführt nachdem doInBackground fertig ist
                        launch_state_6(); // Optionsmenü anzeigen
                    }
                }
                .execute(null,null,null); // Task starten; keine Übergabeparameter
            }
    ...
    private class CompassListener implements SensorEventListener { // innere Klasse
        ...
        /**aktiviert/deaktiviert Compass für grafische Ausgabe
        ...
        /**aktiviert/deaktiviert Compass durch CheckBox im Optionsmenü
            ...
            if (enabledByOptions && enabled) { // wenn Compass durch Benutzer && App aktiviert
                ...
                if (Math.abs(f_new - f_old) > 3.5) { // Hysterese, damit Bild an der Schaltschwelle nicht hin- und herdreht
                    ...
                    if (output!=null) { // Instanz von grafischer Ausgabe vorhanden
                        output.set_degree(f_old + OffsetCorr); // neuen Winkel an Ausgabe übergeben
